...
export default function FinancialDashboard() {
  ...
  // --- FUNÇÃO DE REMOVER COMPROVANTE ---
  const handleRemoveReceipt = (e: React.MouseEvent) => {
      e.preventDefault(); // Não deixa o formulário enviar
      e.stopPropagation(); // Não deixa abrir a janela de seleção de arquivos
  ...
  const handleSubmit = async () => {
    ...
    // Definimos explicitamente que pode ser string OU null
    ...
    if ((formMode === 'installment' || formMode === 'fixed_expense') && editingId) {
        ...
        // Se for novo, salva o JSON já formatado ou null se não tiver nada
  ...
  const renderTransactions = () => { 
    ...
    if (allItems.length === 0) return <p className="text-gray-500 text-center py-8 italic text-sm border border-dashed border-gray-800 rounded-xl">Nenhuma movimentação neste mês.</p>; 
        ...
        return ( 
            <div key={`${item.isFixed ? 'fix' : 'var'}-${item.id}`} className={`flex justify-between items-center p-4 border rounded-xl group transition ${isDimmed ? 'bg-[#0f1219]/50 border-gray-800/50 opacity-60' : 'bg-[#0f1219] border-gray-800 hover:border-gray-700'}`}> 
                <div className="flex items-center gap-4"> 
                    {!item.isFixed && (<button onClick={() => togglePaid('transactions', item.id, item.is_paid)} className={`rounded-full p-1.5 border transition ${item.is_paid ? 'bg-emerald-500/20 border-emerald-500 text-emerald-500' : 'border-gray-600 text-transparent hover:border-emerald-500'}`}><Check size={12} /></button>)} 
                    {item.isFixed && (<button onClick={() => toggleSkipMonth(item)} className={`rounded-full p-1.5 border transition ${item.isSkipped ? 'bg-gray-800 border-gray-700 text-gray-500' : 'border-cyan-500/50 text-cyan-400 hover:bg-cyan-500/10'}`}>{item.isSkipped ? <EyeOff size={12}/> : <Eye size={12}/>}</button>)} 
                    <div> 
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {currentReceipt && (<a href={currentReceipt} target="_blank" rel="noopener noreferrer" className="text-cyan-500 hover:text-cyan-400 flex items-center text-[10px] gap-1 bg-cyan-900/20 px-1.5 rounded transition"><ExternalLink size={10}/> Ver Comprovante</a>)}
                        </div> 
                    </div> 
                </div> 
                <div className="flex items-center gap-3"> 
                    ...
                    <div className="flex gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition"> 
                        {!item.isFixed && item.type === 'expense' && (<button onClick={() => toggleDelay(item.isFixed ? 'recurring' : 'transactions', item)} className="p-1.5 rounded bg-orange-500/10 text-orange-500 hover:bg-orange-500 hover:text-white" title="Stand-by"><Clock size={14}/></button>)} 
                        <button onClick={() => handleEdit(item, item.isFixed ? (item.type === 'income' ? 'income' : 'fixed_expense') : (item.type === 'income' ? 'income' : 'expense'))} className="p-1.5 rounded hover:bg-blue-500/10 text-blue-500"><Pencil size={14}/></button> 
                        <button onClick={() => handleDelete(item.isFixed ? 'recurring' : 'transactions', item.id)} className="p-1.5 rounded hover:bg-red-500/10 text-red-500"><Trash2 size={14}/></button> 
                    </div> 
                </div> 
            </div> 
        ); 
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-100 p-4 md:p-8 font-sans selection:bg-cyan-500 selection:text-black relative">
                            ...
                            return (
                                <tr key={`desk-inst-${inst.id}`} className={`border-b border-gray-800/50 group transition ${isPaid ? 'bg-emerald-950/10' : 'hover:bg-gray-800/30'}`}>
                                    ...
                                    <td className="py-4 text-right flex gap-3 justify-end">
                                        {currentReceipt && (<a href={currentReceipt} target="_blank" rel="noopener noreferrer" className="text-cyan-500 hover:text-white transition"><ExternalLink size={16}/></a>)}
                                        <button onClick={() => toggleDelay('installments', inst)} className="text-gray-600 hover:text-orange-500 opacity-0 group-hover:opacity-100 transition"><Clock size={16}/></button>
                        ...
                        {recurring.filter(r => r.type === 'expense').map((rec) => {
                             ...
                             return (
                                <tr key={`desk-rec-${rec.id}`} className={`border-b border-gray-800/50 group transition ${isPaid ? 'bg-emerald-950/10' : 'hover:bg-gray-800/30'}`}>
                                    ...
                                    <td className="py-4 text-right flex gap-3 justify-end">
                                        {currentReceipt && (<a href={currentReceipt} target="_blank" rel="noopener noreferrer" className="text-cyan-500 hover:text-white transition"><ExternalLink size={16}/></a>)}
                                        <button onClick={() => toggleDelay('recurring', rec)} className="text-gray-600 hover:text-orange-500 opacity-0 group-hover:opacity-100 transition"><Clock size={16}/></button>
      ...
      {isFormOpen && (<div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="bg-[#111] border border-gray-700 p-8 rounded-3xl w-full max-w-md shadow-2xl relative max-h-[90vh] overflow-y-auto"><button onClick={() => setIsFormOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-white"><X size={24} /></button><h2 className="text-2xl font-bold mb-6 text-white">{editingId ? 'Editar' : 'Novo Lançamento'}</h2>
            ...
            {/* AREA DE UPLOAD DE COMPROVANTE ATUALIZADA */}
            {((formMode !== 'installment' && formMode !== 'fixed_expense') || editingId) && (
                <div className="border border-dashed border-gray-700 rounded-xl p-4 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-gray-900/50 transition relative group">
                    {/* O input só funciona se NÃO tiver comprovante. Se tiver, ele fica 'invisível' pra não atrapalhar o botão de excluir */}
                    {!formData.receiptUrl && (
