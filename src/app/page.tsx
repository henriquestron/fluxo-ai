...
import { CreditCard, TrendingDown, DollarSign, Plus, X, List, LayoutGrid, Sparkles, Send, Trash2, AlertCircle, CheckCircle2, Pencil, Clock, AlertTriangle, Check, LogIn, LogOut, User, Eye, EyeOff, CheckSquare, Square, ArrowRight } from 'lucide-react';
...
export default function FinancialDashboard() {
  // Define o mês atual real do sistema para comparações
  const currentSystemMonthIndex = new Date().getMonth(); // 0 = Jan, 1 = Fev...
  ...
  // NOVO: Modal de Contas Atrasadas (Virada de Mês)
  ...
  // Effect para checar contas atrasadas assim que os dados carregam
  useEffect(() => {
  ...
  // --- LÓGICA DE VIRADA DE MÊS (CHECK FOR PAST DUE) ---
  const checkForPastDueItems = () => {
      const overdueItems: any[] = [];
      
      // Varre meses passados até o mês atual (não incluso)
      for (let i = 0; i < currentSystemMonthIndex; i++) {
          ...
          // 1. Checa Transações Avulsas (Despesas) não pagas
          transactions.forEach(t => {
              if (t.type === 'expense' && t.date?.includes(dateFilter) && !t.is_paid && t.status !== 'delayed') {
                  overdueItems.push({ ...t, origin: 'transactions', month: pastMonthName });
              }
          });

          // 2. Checa Parcelas não pagas
          installments.forEach(inst => {
              const pastInstNum = inst.current_installment + i;
              if (pastInstNum >= 1 && pastInstNum <= inst.installments_count) {
                  if (!inst.paid_months?.includes(pastMonthName) && inst.status !== 'delayed') {
                      overdueItems.push({ ...inst, origin: 'installments', month: pastMonthName, amount: inst.value_per_month });
                  }
              }
          });

          // 3. Checa Fixos não pagos
          recurring.forEach(rec => {
              if (rec.type === 'expense' && !rec.paid_months?.includes(pastMonthName) && !rec.skipped_months?.includes(pastMonthName) && rec.status !== 'delayed') {
                  overdueItems.push({ ...rec, origin: 'recurring', month: pastMonthName, amount: rec.value });
              }
          });
      }

      // Se achou itens, e o modal ainda não foi aberto/fechado nesta sessão (opcional), abre.
      if (overdueItems.length > 0) {
          setPastDueItems(overdueItems);
          // Pequeno timeout para não abrir na cara logo que carrega
          setTimeout(() => setIsRolloverModalOpen(true), 1000);
      }
  };

  // --- ACTIONS ---
  
  const handleLoginPassword = async () => { /* ... Lógica igual ... */ setLoadingAuth(true); const { error } = await supabase.auth.signInWithPassword({ email, password }); if (error) setAuthMessage("Erro: " + error.message); else { setAuthMessage("Login realizado!"); setIsAuthModalOpen(false); window.location.reload(); } setLoadingAuth(false); };
  ...
  // Mover para Standby (Funciona para todos os tipos agora)
  const toggleDelay = async (table: string, item: any) => {
      const newStatus = item.status === 'delayed' ? 'active' : 'delayed';
      if (user) {
          await supabase.from(table).update({ status: newStatus }).eq('id', item.id);
          loadData(user);
      } else {
          // Lógica local para mudar status
          const updateStatus = (list: any[]) => list.map(i => i.id === item.id ? { ...i, status: newStatus } : i);
          if (table === 'transactions') saveDataLocal(updateStatus(transactions), installments, recurring);
          else if (table === 'installments') saveDataLocal(transactions, updateStatus(installments), recurring);
          else saveDataLocal(transactions, installments, updateStatus(recurring));
      }
      
      // Se estivermos no modal de virada de mês, remove o item da lista visual
      if (isRolloverModalOpen) {
          setPastDueItems(prev => prev.filter(i => i.id !== item.id || i.origin !== table));
      }
  };
  ...
  const getMonthData = (monthName: string) => {
    ...
    // Despesa (Exclui se status for 'delayed')
    ...
    const expenseFixed = recurring.filter(r => r.type === 'expense' && !r.skipped_months?.includes(monthName) && r.status !== 'delayed').reduce((acc, curr) => acc + curr.value, 0);
    
    const installTotal = installments.reduce((acc, curr) => {
        if (curr.status === 'delayed') return acc; // Ignora se estiver em standby
    ...
    // Soma TODAS as contas em standby (Avulsas, Fixas, Parcelas)
    const delayedTransactions = transactions.filter(t => t.status === 'delayed').reduce((acc, curr) => acc + curr.amount, 0);
    const delayedInstallments = installments.filter(i => i.status === 'delayed').reduce((acc, curr) => acc + curr.value_per_month, 0); // Soma valor mensal
    const delayedRecurring = recurring.filter(r => r.status === 'delayed' && r.type === 'expense').reduce((acc, curr) => acc + curr.value, 0);
    const delayedTotal = delayedTransactions + delayedInstallments + delayedRecurring;

    // Bola de Neve (Atrasados não pagos e não standby)
    ...
    for (let i = 0; i < monthIndex; i++) {
        ...
        recurring.filter(r => r.type === 'expense').forEach(rec => {
            if (rec.status !== 'delayed' && !rec.paid_months?.includes(pastMonth) && !rec.skipped_months?.includes(pastMonth)) {
    ...
    return { 
        income: incomeFixed + incomeVariable, 
        expenseTotal: totalObligations, 
        accumulatedDebt, 
        balance: (incomeFixed + incomeVariable) - totalObligations, 
        delayedTotal 
    };
  };
  ...
  const renderTransactions = () => {
     ...
     const fixedItems = recurring.map(r => ({ ...r, isFixed: true, isSkipped: r.skipped_months?.includes(activeTab), date: 'Fixo Mensal', amount: r.value }));
     ...
     return allItems.map((item) => {
        // Se estiver em Standby (Fixos podem ter status), não mostra aqui se não for a aba de standby
        ...
        return (
            <div key={`${item.isFixed ? 'fix' : 'var'}-${item.id}`} className={`flex justify-between items-center p-4 border rounded-xl group transition ${isDimmed ? 'bg-gray-900/20 border-gray-800 opacity-50' : 'bg-gray-900/40 border-gray-800 hover:bg-gray-900/60'}`}>
                <div className="flex items-center gap-4">
                    {!item.isFixed && (<button onClick={() => togglePaid('transactions', item.id, item.is_paid)} className={`rounded-full p-1 border ${item.is_paid ? 'bg-green-500 border-green-500 text-white' : 'border-gray-600 text-transparent hover:border-green-500'}`}><Check size={12} /></button>)}
                    {item.isFixed && (<button onClick={() => toggleSkipMonth(item)} className={`rounded-full p-1 border ${item.isSkipped ? 'bg-gray-700 border-gray-700 text-gray-400' : 'border-purple-500 text-purple-400 hover:bg-purple-500 hover:text-white'}`}>{item.isSkipped ? <EyeOff size={12}/> : <Eye size={12}/>}</button>)}
                    <div><p className={`font-semibold text-sm ${isDimmed ? 'text-gray-500 line-through' : 'text-gray-200'}`}>{item.title} {item.isFixed && <span className="text-[10px] bg-gray-800 px-1 rounded ml-1 text-gray-400">FIXO</span>}</p><p className="text-xs text-gray-500">{item.isSkipped ? 'PULADO' : item.date}</p></div>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                        {item.type === 'expense' && (<button onClick={() => toggleDelay(item.isFixed ? 'recurring' : 'transactions', item)} className="p-1.5 rounded bg-orange-500/10 text-orange-500 hover:bg-orange-500 hover:text-white" title="Stand-by"><Clock size={14}/></button>)}
  ...
  const renderDelayed = () => {
      // Pega tudo que está delayed
      const delayedTrans = transactions.filter(t => t.status === 'delayed');
      const delayedRecur = recurring.filter(r => r.status === 'delayed' && r.type === 'expense');
      const delayedInst = installments.filter(i => i.status === 'delayed');
      
      const allDelayed = [...delayedTrans, ...delayedRecur, ...delayedInst];
      ...
      return (
        <div className="mt-8 border border-red-900/50 bg-red-950/10 rounded-2xl p-6">
            ...
            <div className="space-y-2">
                {allDelayed.map((item: any) => (
                    <div key={`delayed-${item.id}`} className="flex justify-between items-center p-3 bg-red-900/20 rounded-lg border border-red-900/30">
                        <span className="text-red-200 text-sm">{item.title}</span>
                        <div className="flex items-center gap-3">
                            <span className="font-mono text-red-400 font-bold">R$ {item.amount || item.value || item.value_per_month}</span>
                            <button onClick={() => toggleDelay(item.date ? 'transactions' : item.installments_count ? 'installments' : 'recurring', item)} className="text-xs bg-red-500 text-white px-2 py-1 rounded hover:bg-red-400">Restaurar</button>
                        </div>
                    </div>
                ))}
            </div>
            <p className="text-xs text-red-500/60 mt-3 text-center">Estes valores NÃO estão sendo descontados do saldo atual.</p>
        </div>
      )
  };
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-gray-100 p-4 md:p-8 font-sans selection:bg-purple-500 selection:text-white relative">
      <header className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1"><div className={`w-2 h-2 rounded-full ${user ? 'bg-green-500' : 'bg-yellow-500 animate-pulse'}`}></div><p className="text-gray-500 text-sm">{user ? 'Online' : 'Local'}</p></div>
        </div>
      ...
      <div className={`grid gap-6 mb-12 ${gridClass}`}>
        ...
        <Card title="A Pagar (Mês + Atrasados)" value={currentMonthData.expenseTotal.toFixed(2)} icon={TrendingDown} type="expense" subValueLabel={currentMonthData.accumulatedDebt > 0 ? (<span className="text-red-400 font-bold flex items-center gap-1"><AlertCircle size={12}/> + R$ {currentMonthData.accumulatedDebt.toFixed(2)} Atrasado</span>) : null} />
        <Card title="Entradas" value={currentMonthData.income.toFixed(2)} icon={CreditCard} type="income" />
        {hasDelayed && (<div className="bg-red-900/20 backdrop-blur-md border border-red-500/50 p-6 rounded-2xl flex flex-col justify-between relative h-full"><div><div className="flex items-center gap-3 text-red-400 mb-2"><Clock size={24} /><h3 className="font-bold">Em Stand-by</h3></div><p className="text-white text-2xl font-bold">R$ {currentMonthData.delayedTotal.toFixed(2)}</p></div><p className="text-xs text-red-400/60 mt-2">Congeladas</p></div>)}
      </div>
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="text-sm">
                        {installments.map((inst) => {
                            if (inst.status === 'delayed') return null; // Esconde se estiver em standby
                            ...
                            return (
                                <tr key={inst.id} className={`border-b border-gray-800/50 group transition ${isPaid ? 'bg-green-900/10' : 'hover:bg-gray-800/20'}`}>
                                    ...
                                    <td className="py-4 text-right"><button onClick={() => togglePaidMonth('installments', inst)} className={`p-1 rounded transition ${isPaid ? 'text-green-400 hover:text-green-300' : 'text-gray-600 hover:text-white'}`}>{isPaid ? <CheckSquare size={20}/> : <Square size={20}/>}</button></td>
                                    <td className="py-4 text-right flex gap-2 justify-end">
                                        {/* Botão Standby na Tabela */}
                                        <button onClick={() => toggleDelay('installments', inst)} className="text-gray-600 hover:text-orange-500 opacity-0 group-hover:opacity-100 transition" title="Mover para Standby"><Clock size={16}/></button>
                                        <button onClick={() => handleEdit(inst, 'installment')} className="text-gray-600 hover:text-blue-400 opacity-0 group-hover:opacity-100 transition"><Pencil size={16}/></button>
                                        <button onClick={() => handleDelete('installments', inst.id)} className="text-gray-600 hover:text-red-500 opacity-0 group-hover:opacity-100 transition"><Trash2 size={16}/></button>
                                    </td>
                                </tr>
                            );
                        })}
                        {recurring.filter(r => r.type === 'expense').map((rec) => {
                             ...
                             return (
                                <tr key={`rec-${rec.id}`} className={`border-b border-gray-800/50 group transition ${isPaid ? 'bg-green-900/10' : 'hover:bg-gray-800/20'}`}>
                                    ...
                                    <td className="py-4 text-right"><button onClick={() => togglePaidMonth('recurring', rec)} className={`p-1 rounded transition ${isPaid ? 'text-green-400 hover:text-green-300' : 'text-gray-600 hover:text-white'}`}>{isPaid ? <CheckSquare size={20}/> : <Square size={20}/>}</button></td>
                                    <td className="py-4 text-right flex gap-2 justify-end">
                                        <button onClick={() => toggleDelay('recurring', rec)} className="text-gray-600 hover:text-orange-500 opacity-0 group-hover:opacity-100 transition" title="Mover para Standby"><Clock size={16}/></button>
                                        <button onClick={() => handleEdit(rec, 'fixed_expense')} className="text-gray-600 hover:text-blue-400 opacity-0 group-hover:opacity-100 transition"><Pencil size={16}/></button>
                                        <button onClick={() => handleDelete('recurring', rec.id)} className="text-gray-600 hover:text-red-500 opacity-0 group-hover:opacity-100 transition"><Trash2 size={16}/></button>
                                    </td>
                                </tr>
                            );
                        })}
      ...
      {/* MODAL ALERTA VIRADA DE MÊS */}
      {isRolloverModalOpen && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50 p-6">
            <div className="bg-[#111] border border-gray-700 p-8 rounded-3xl w-full max-w-lg shadow-2xl relative">
                <h2 className="text-2xl font-bold mb-2 text-white flex items-center gap-2"><AlertCircle className="text-orange-500"/> Contas em Aberto</h2>
                <p className="text-gray-400 text-sm mb-6">Existem contas de meses passados que você não marcou como pagas. O que deseja fazer com elas?</p>
                
                <div className="max-h-[300px] overflow-y-auto space-y-2 mb-6 pr-2">
                    {pastDueItems.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center p-3 bg-gray-900 rounded-lg border border-gray-800">
                            <div>
                                <p className="text-white font-medium text-sm">{item.title}</p>
                                <p className="text-xs text-gray-500">{item.month} • {item.origin === 'installments' ? 'Parcela' : 'Conta'}</p>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="text-red-400 font-mono">R$ {item.amount || item.value || item.value_per_month}</span>
                                <button onClick={() => toggleDelay(item.origin, item)} className="text-xs bg-orange-500/20 text-orange-400 px-2 py-1 rounded border border-orange-500/50 hover:bg-orange-500 hover:text-white transition">Mover p/ Stand-by</button>
                            </div>
                        </div>
                    ))}
                </div>
